...
+ (void)load {
	...
	// We use NSSelectorFromString, because ARC forbids using 'dealloc' inside @selector() block.
	[self brc_swizzleMethod:NSSelectorFromString(@"dealloc") withMethod:@selector(brc_dealloc)];
}
